...
//					+ sLockPasswordFilename);
	...
	public static boolean checkHasPatternPwd(Context ctx) {//App.getInstance()).getLockPatternUtils()
		String phone = new SharedPreferencesData(ctx).getValue("phone");
		String user0 = new SharedPreferencesData(ctx).getOtherValue("user0");
		if (SettingUtils.getInstance(App.getInstance()).isPatternPwdOn(user0) && (new LockPatternUtils(ctx,phone).savedPatternExists())) {
//			Intent i = new Intent(ctx, UnlockGesturePasswordActivity.class);
//			ctx.startActivity(i);
            return true;
		}
        return false;
