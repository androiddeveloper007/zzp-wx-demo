package library.widget.progressbarcg;
...
public class FloatTextProgressBar extends ProgressBar {
    ...
    protected void getDimension() {
        super.getDimension();
        progressHeight = height / 9;
        floatRectWidth = height / 5 * 6;
        floatRectHeight = height / 9 * 5;
    ...
    private void drawFloatRect(Canvas canvas){
        if (progressWidth < floatRectWidth/2){
            ...
            Path path = new Path();
            path.moveTo(margin + floatRectWidth / 2 - triangleWidth / 2, height / 5*2);
            path.lineTo(margin + floatRectWidth / 2 + triangleWidth / 2, height / 5*2);
            path.lineTo(margin + floatRectWidth / 2, floatRectWidth / 4 + height / 5*2);
            path.close();
            ...
            paint.setColor(triangleColor);
            Path path = new Path();
            path.moveTo(width - margin - floatRectWidth / 2 - triangleWidth / 2, height / 5*2);
            path.lineTo(width - margin - floatRectWidth / 2 + triangleWidth / 2, height / 5*2);
            path.lineTo(width - margin - floatRectWidth / 2, floatRectWidth / 4 + height / 5*2);
            path.close();
            canvas.drawPath(path, paint);
        } else {
            //绘制浮动框
            ...
            paint.setColor(triangleColor);
            Path path = new Path();
            path.moveTo(progressWidth - triangleWidth / 2, height / 5*2);
            path.lineTo(progressWidth + triangleWidth / 2, height / 5*2);
            path.lineTo(progressWidth, floatRectWidth / 4 + height / 5*2);
            path.close();
            canvas.drawPath(path, paint);
        }
    }
    ...
    public void drawText(Canvas canvas) {
        ...
        float textWidth = paint.measureText(p + "%");
        if (progressWidth < floatRectWidth/2 + margin) {
            canvas.drawText(p + "%", margin + floatRectWidth / 2 - textWidth / 2, floatRectHeight / 2 + textSize / 4, paint);
        } else if (width - progressWidth < floatRectWidth/2 + margin){
            canvas.drawText(p + "%", width - margin - floatRectWidth / 2 - textWidth / 2, floatRectHeight / 2 + textSize / 4, paint);
        }
//        else if(width == progressWidth+floatRectWidth/2){
//            canvas.drawText(progress0 + "%", progressWidth - textWidth / 2, floatRectHeight / 2 + textSize / 4, paint);
//        }
        else {
            canvas.drawText(p + "%", progressWidth - textWidth / 2, floatRectHeight / 2 + textSize / 4, paint);
        }
    }
